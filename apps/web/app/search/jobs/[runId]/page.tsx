...
import { fetchEmbeddingRun, fetchEmbeddingRunEvents } from "@/lib/api";
...
export default async function EmbeddingJobPage({
  params,
}: {
  params: { runId: string };
}) {
  const run = await fetchEmbeddingRun(params.runId);
  if (!run) notFound();

  const events = await fetchEmbeddingRunEvents(run.id);

  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <Link
        href="/search/explorer"
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold tracking-tight">Embedding job</h2>
      ...
      <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Stat label="Embedded" value={run.documents_embedded} />
        <Stat label="Skipped" value={run.documents_skipped} />
